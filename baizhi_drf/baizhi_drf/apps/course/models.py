...
from course.BaseModels import BaseModel
...
class Course(BaseModel):
    ...
    course_video = models.FileField(upload_to="video", null=True, verbose_name="视频")
    ...
    @property
    def brief_html(self):
        brief = self.brief.replace('src="/media', 'src="%s/media' % "http://api.abc.com:8000")
        return brief
    ...
    def real_nuw_price(self, expire_id=0):
        nuw_price = self.price
        try:
            if expire_id > 0:
                nuw_price = CourseExpire.objects.get(id=expire_id).price
